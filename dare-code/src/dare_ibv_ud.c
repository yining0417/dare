...
static int
handle_rc_syn_back(struct ibv_wc *wc, rc_syn_t *msg, uint8_t* raw);
static int
...
static int
wc_to_ud_ep_back(ud_ep_t *ud_ep, struct ibv_wc *wc, uint8_t* raw);
static int
...
struct ibv_ah* ud_ah_create_back( uint16_t dlid, uint8_t* raw )
{
    struct ibv_ah *ah = NULL;
    struct ibv_ah_attr ah_attr;
...
handle_message_from_client_back( struct ibv_wc *wc, ud_hdr_t *ud_hdr, uint8_t* raw )
{
    ...
    switch(type) {
        ...
        case RC_SYN:
        {
            ...
            //info(log_fp, ">> Received RC_SYN from lid%"PRIu16"\n", wc->slid);
            type = MSG_NONE;
            rc = handle_rc_syn_back(wc, (rc_syn_t*)ud_hdr,raw);
            if (0 != rc) {
                error(log_fp, "Cannot handle RC_SYN msg\n");
                type = MSG_ERROR;
            }
...
wc_to_ud_ep_back(ud_ep_t *ud_ep, struct ibv_wc *wc, uint8_t* raw)
{
    ...
    ud_ep->ah = ud_ah_create_back(ud_ep->lid, raw);
    if (NULL == ud_ep->ah) {
        error_return(1, log_fp, "Cannot create AH for LID %"PRIu16"\n",
                     ud_ep->lid);
    }
